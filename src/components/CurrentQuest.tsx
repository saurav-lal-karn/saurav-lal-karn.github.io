...
import { Layers, ArrowRight, Shield, CheckCircle, Database, Server, Smartphone, Cpu, HelpCircle } from 'lucide-react';
...
const diagramNodes: DiagramNode[] = [
  {
    ...
    details: 'Leverages Vite for blazing fast builds, Tailwind CSS for fluid, highly responsive layouts, and unified state machines for tracking entries, filtering expenses, and rendering dynamic analytical graphs.',
    icon: Smartphone
  },
  {
    id: 'backend',
    ...
    details: 'Written in pure Go utilizing the Fiber routing framework. Provides lightning-fast response times, custom session authentications, input parsing validations, and initiates parallel processing queues.',
    icon: Server
  },
  {
    id: 'postgres',
    ...
    details: 'Maintains transaction ledger integrity. Integrates custom indexes, primary-foreign constraints, and optimized queries to prevent locks under heavy transaction logging audits.',
    icon: Database
  },
  {
    id: 'ocr',
    ...
    details: 'A lightweight server utilizing Tesseract or Google Cloud Vision API to extract raw string streams, positional lines, and bounds coordinates from uploaded receipt images.',
    icon: Cpu
  },
  {
    id: 'ai',
    ...
    details: 'Uses the Gemini API or LLMs via structured schema matching to parse chaotic OCR outputs into flawless standard objects containing vendor name, category, amount, dates, and currency.',
    icon: Layers
  }
];
...
export default function CurrentQuest() {
  ...
  const currentNodeInfo = diagramNodes.find(n => n.id === (hoveredNode || selectedNode)) || diagramNodes[0];

  return (
    <div id="quest-section" className="py-16 px-4 md:px-8 border-b border-ink-faded/15 manuscript-lines relative">
      <div className="max-w-4xl mx-auto">
        
        {/* Section Heading */}
        <div className="text-center mb-12">
          <div className="font-cursive text-3xl text-gold-ancient/90 mb-1">Current Quest</div>
          ...
          <p className="mt-4 text-sm md:text-base font-serif-antique text-ink-faded italic max-w-lg mx-auto">
            "Forging an automated, intelligent finance ledger for absolute fiscal clarity."
          </p>
        </div>
        ...
        <div className="bg-parchment-light/70 border border-gold-ancient/30 rounded-lg p-6 md:p-8 parchment-shadow mb-10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-ink-faded/70 font-serif-antique italic">Active Engineering Quest</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="border-l-2 border-red-800/30 pl-4 py-1">
              ...
              <p className="font-serif-antique text-sm text-ink-faded leading-relaxed">
                Manually logging paper receipts is tedious, error-prone, and unsustainable. Standard trackers lack natural automation, requiring users to self-classify every purchase structure manually.
              </p>
            </div>
            <div className="border-l-2 border-green-800/30 pl-4 py-1">
              ...
              <p className="font-serif-antique text-sm text-ink-faded leading-relaxed">
                A dual-backend framework where receipt uploads trigger quick OCR extractions and secure AI processing. Chaos is instantly mapped into reliable structured logs aligned in Go routines.
              </p>
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-3 bg-parchment-light rounded border border-gold-ancient/10">
                <span className="font-mono-antique text-[10px] text-gold-ancient font-bold block mb-1">FE-01</span>
                <span className="font-display-antique text-sm text-ink-dark font-medium block mb-1">Instant OCR Uploads</span>
                <span className="font-serif-antique text-xs text-ink-faded block">Drag receipts, scrape coordinates, and extract text streams cleanly.</span>
              </div>
              <div className="p-3 bg-parchment-light rounded border border-gold-ancient/10">
                <span className="font-mono-antique text-[10px] text-gold-ancient font-bold block mb-1">BE-02</span>
                <span className="font-display-antique text-sm text-ink-dark font-medium block mb-1">Structured AI Parser</span>
                <span className="font-serif-antique text-xs text-ink-faded block">Structured models mapping chaotic invoices with token efficiency.</span>
              </div>
              <div className="p-3 bg-parchment-light rounded border border-gold-ancient/10">
                <span className="font-mono-antique text-[10px] text-gold-ancient font-bold block mb-1">BE-03</span>
                <span className="font-display-antique text-sm text-ink-dark font-medium block mb-1">Sub-Sec Go Routines</span>
                <span className="font-serif-antique text-xs text-ink-faded block">Go-based REST pipelines handling requests with zero allocation pressure.</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-center font-serif-antique text-xs text-ink-faded italic mb-4">
              Hover/Click nodes to reveal parchment details and engineering trade-offs.
            </p>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-10 p-4 border border-gold-ancient/20 rounded bg-parchment-light/35 mb-6">
              ...
              <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 w-full justify-around py-4">
                ...
                <div className="flex flex-col items-center gap-4">
                  <span className="font-mono-antique text-[10px] text-gold-ancient uppercase tracking-wider">Client</span>
                  <button
                  ...
                  >
                    ...
                    <span className="font-display-antique text-sm text-ink-dark font-bold block">React Frontend</span>
                    <span className="font-mono-antique text-[9px] text-ink-faded/80 block uppercase">TypeScript + Vite</span>
                  </button>
                </div>
                ...
                <div className="flex flex-col items-center gap-4">
                  <span className="font-mono-antique text-[10px] text-gold-ancient uppercase tracking-wider">Gateway</span>
                  <button
                  ...
                  >
                    ...
                    <span className="font-display-antique text-sm text-ink-dark font-bold block">Go Backend</span>
                    <span className="font-mono-antique text-[9px] text-ink-faded/80 block uppercase">Fiber REST API</span>
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex flex-col items-center">
                    <span className="font-mono-antique text-[10px] text-gold-ancient uppercase tracking-wider mb-1">State</span>
                    <button
                      ...
                      className={`w-40 p-3 rounded border text-center transition-all duration-300 ${
                        (hoveredNode === 'postgres' || selectedNode === 'postgres')
                          ? 'bg-gold-ancient/15 border-gold-ancient shadow-md scale-105'
                          : 'bg-parchment-light border-gold-ancient/30 hover:border-gold-ancient/70'
                      }`}
                    >
                      <Database className="w-5 h-5 mx-auto text-gold-ancient mb-1.5" />
                      <span className="font-display-antique text-sm text-ink-dark font-bold block">PostgreSQL</span>
                      <span className="font-mono-antique text-[9px] text-ink-faded/80 block uppercase">Relational SQL</span>
                    </button>
                  </div>

                  {/* Parallel OCR & AI */}
                  <div className="flex flex-col items-center gap-2 border-t border-dashed border-gold-ancient/30 pt-3">
                    <span className="font-mono-antique text-[9px] text-gold-ancient uppercase tracking-wider">AI Service Mesh</span>
                    <div className="flex gap-2">
                      ...
                      >
                        <span className="font-display-antique text-xs text-ink-dark font-bold block">Python OCR</span>
                      </button>
                      <button
                      ...
                      >
                        <span className="font-display-antique text-xs text-ink-dark font-bold block">AI Engine</span>
                      </button>
                    </div>
                  </div>
                </div>
          ...
          <div className="mt-8 border-t border-ink-faded/10 pt-6">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {['React.js', 'Go (Fiber)', 'PostgreSQL', 'Python (OCR)', 'Gemini Model API', 'Docker'].map((tech) => (
                <span
                  key={tech}
